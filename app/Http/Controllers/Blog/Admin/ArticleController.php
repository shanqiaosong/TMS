...
class ArticleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $articles = Article::when(isset($status), function ($query) use ($status) {
                    ...
                    })->orderBy('is_top','DESC')->paginate($request->pagesize);

        for ($i=0; $i < sizeof($articles); $i++) {
            $articles[$i]->key = $articles[$i]->id;
            $articles[$i]->content_html_short = \Illuminate\Support\Str::limit(strip_tags($articles[$i]->content_html), 60);
    ...
    public function markdown(Request $request)
    {
        ...
        return $converter->convert($request->myContent);
    }

...
import { Form, Input, Button, message, Spin, Upload, Alert } from 'antd';
      ...
      return (
        <Form onSubmit={this.handleSubmit} style={{ paddingTop:20 }}>
          <FormItem rules={[{
              required: true,
              message: '昵称不能为空！',
          }]} name={'name'} initialValue={formData.name} {...formItemLayout} label="昵称">

              <Input placeholder="请输入昵称" />
          </FormItem>
          <FormItem {...formItemLayout} name={'email'} rules={[{
              required: true,
              message: '邮箱不能为空！'
          },{
              type: 'email',
              message: '邮箱格式不正确！'
          }]} initialValue={formData.email} label="邮箱" extra={<Alert message="更换邮箱后，登录需使用新邮箱" type="warning" showIcon />}>

              <Input placeholder="请输入邮箱" />

          </FormItem>
          <FormItem name={'avatar'} valuePropName={'fileList'} getValueFromEvent={this.normFile} {...formItemLayout} label="头像">
